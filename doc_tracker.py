...
with st.form("add_doc"):
    ...
    if submitted:
        if (not file_name or not doc_ref or not title or
            ...
            if not status:
                st.info("⚠️ لم يتم اختيار الحالة، اخترها الآن:")
                selected_status = st.selectbox("📌 اختر الحالة الآن:", status_options, key="post_status")
                if st.button("✅ اعتماد الحالة الآن"):
                    df.at[len(df)-1, "Status"] = selected_status
                    df.to_csv("documents.csv", index=False)
                    st.success("✅ تم اعتماد الحالة بنجاح")
                    st.rerun()
            else:
                st.rerun()
